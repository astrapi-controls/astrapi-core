...
def _render_content(request) -> str:
    ...
    return render_string(request, f"{KEY}/content.html", _list_ctx())
...
def _list_ctx() -> dict:
    from astrapi_core.modules.scheduler.engine import list_jobs, get_registered_actions
    return {
        "jobs":         list_jobs(),
...
def scheduler_content(request: Request):
    return render(request, f"{KEY}/content.html", _list_ctx())
...
async def scheduler_job_save(job_id: str, request: Request):
    from astrapi_core.modules.scheduler.engine import update_job, get_registered_actions
    form = await request.form()
