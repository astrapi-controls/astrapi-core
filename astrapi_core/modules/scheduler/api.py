...
def list_jobs():
    from astrapi_core.modules.scheduler.engine import list_jobs as _list
    return {"jobs": _list()}
...
def get_job(job_id: str):
    from astrapi_core.modules.scheduler.engine import get_job as _get
    job = _get(job_id)
    if job is None:
...
def create_job(job_id: str, data: JobIn):
    from astrapi_core.modules.scheduler.engine import create_job as _create
    try:
...
def update_job(job_id: str, data: JobIn):
    from astrapi_core.modules.scheduler.engine import update_job as _update, get_job as _get
...
def delete_job(job_id: str):
    from astrapi_core.modules.scheduler.engine import delete_job as _delete, get_job as _get
    if _get(job_id) is None:
        raise HTTPException(404, f"Job '{job_id}' nicht gefunden")
    _delete(job_id)
...
def trigger_job(job_id: str):
    from astrapi_core.modules.scheduler.engine import trigger_job as _trigger, get_job as _get
    if _get(job_id) is None:
        raise HTTPException(404, f"Job '{job_id}' nicht gefunden")
    _trigger(job_id)
    return {"ok": True}
...
def toggle_job(job_id: str):
    from astrapi_core.modules.scheduler.engine import toggle_job as _toggle, get_job as _get
    if _get(job_id) is None:
        raise HTTPException(404, f"Job '{job_id}' nicht gefunden")
    _toggle(job_id)
    return _get(job_id)
